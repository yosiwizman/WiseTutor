...
- **Real audible Piper output (speakers producing sound): Tier 3** — A human has NOT confirmed audio in this session. The WAV file is structurally valid (RIFF/PCM/16-bit/mono 16000 Hz) and the backend synthesis pipeline is confirmed working end-to-end, but subjective playback through speakers by a human has not occurred. This remains Tier 3 until a human independently plays the audio and confirms it is audible.
