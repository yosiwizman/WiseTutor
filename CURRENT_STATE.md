...
**Mobile-responsive workspace (2026-04-14):** at viewports narrower than 768 px (`md`), the workspace sidebar becomes a fixed-position slide-out drawer (`w-[260px]`, `-translate-x-full` when closed, `translate-x-0` when open) instead of taking a 220 px flex column. A `data-testid="mobile-nav-toggle"` hamburger appears at the top-left of `<main>`, and a `data-testid="mobile-nav-backdrop"` positioned to the right of the drawer closes it on tap. Desktop behavior unchanged via `md:static md:translate-x-0 md:transition-none` + `md:w-[220px]`.

**Real-iPhone-Safari follow-up (2026-04-14, same day):** founder's real-device screenshots after f140397 showed the composer was still clipped by Safari's bottom URL bar and the drawer Settings link sat below the visible viewport. Root cause: `h-screen` = `100vh` on iOS Safari reports the LARGEST viewport height (URL bar hidden), so content positioned at the bottom falls under Safari's chrome when the bar is visible. Fix (narrow):
  - Workspace + utility layouts: `h-screen` → `h-dvh` (dynamic viewport height — follows the actual visible area).
  - `<main>`: `pb-[env(safe-area-inset-bottom)]` to keep composer above the iOS home indicator.
