...
const AssistantMessage = memo(function AssistantMessage({
  ...
  const runtimeMeta = useMemo(() => {
    const ev = msg.events?.find((e) => {
      const m = e.metadata as Record<string, unknown> | undefined;
      return m?.runtime;
    });
    const rt = (ev?.metadata as Record<string, unknown> | undefined)?.runtime as
      | { binding?: string; provider_name?: string; model?: string; base_url?: string }
      | undefined;
    return rt || null;
  }, [msg.events]);
  ...
  return (
    <>
      ...
      {runtimeMeta ? (
        <div
          className="mt-1.5 inline-flex items-center gap-1 rounded-full border border-[var(--border)]/40 bg-[var(--card)]/40 px-2 py-[2px] text-[10px] text-[var(--muted-foreground)]"
          title="Server-recorded runtime for this message (not the model's self-report)"
        >
          <span className="h-1.5 w-1.5 rounded-full bg-emerald-500" />
          <span className="font-mono">{runtimeMeta.provider_name || runtimeMeta.binding || "?"}</span>
          <span>·</span>
          <span className="font-mono">{runtimeMeta.model || "?"}</span>
