...
class AgenticChatPipeline:
    ...
    async def run(self, context: UnifiedContext, stream: StreamBus) -> None:
        # Emit server-side runtime truth at the very start so the UI can render
        # per-message metadata sourced from the actual resolved config — not the
        # model's self-report.
        try:
                ...
                StreamEvent(
                    ...
                    metadata={
                        "runtime": {
                            ...
                            "provider_name": getattr(self.llm_config, "provider_name", "") or self.binding or "",
                            "model": self.model or "",
        ...
        # Identity short-circuit: questions like "what model/provider are you?"
        # are answered DIRECTLY from server runtime metadata — never from the
        # LLM, which has proven willing to parrot poisoned memory as truth
        # (audit 2026-04-12). Reply carries the same runtime chip path.
        if self._is_identity_question(context.user_message):
            import re as _re_local
            from deeptutor.core.stream import StreamEvent, StreamEventType

            provider_disp = {
                "openai": "OpenAI", "anthropic": "Anthropic", "ollama": "Local Ollama",
                "gemini": "Google Gemini", "azure_openai": "Azure OpenAI",
                "deepseek": "DeepSeek", "groq": "Groq", "mistral": "Mistral",
            }.get((self.binding or "").lower(), self.binding or "unknown")
            truthful_answer = (
                f"Answered from server runtime truth (not the model's self-report): "
                f"this turn is handled by **{provider_disp}** using model "
                f"**`{self.model or 'unknown'}`** at `{self.base_url or '—'}`."
            )
            await stream.emit(
                StreamEvent(
                    ...
                    source="chat",
                    stage="responding",
                    content=truthful_answer,
                    metadata={"identity_answered_from_runtime": True},
                )
            )
            await stream.result(
                {
                    "response": truthful_answer,
                    "metadata": {
                        "identity_answered_from_runtime": True,
                        "runtime": {
                            "binding": self.binding,
                            "provider_name": getattr(self.llm_config, "provider_name", "") or self.binding,
                            "model": self.model,
    ...
    # Matches common identity/provider questions. Kept broad for short direct
    # questions (<=400 chars). Examples it must catch:
    #   "what model are you"   "which LLM are you"
    #   "what ai model and provider are you"
    #   "what provider is powering this"
    #   "who powers you"   "are you GPT?"   "are you Claude?"
    #   "what's your model"  "tell me your model"  "identify the model"
    _IDENTITY_QUESTION_RE = __import__("re").compile(
        r"(?i)("
        r"\b(what|which)\b[^.?!\n]{0,60}\b(model|provider|llm|ai)\b[^.?!\n]{0,60}\b(are\s+you|is\s+this|is\s+that|are\s+we|powering|powered|behind)\b|"
        r"\bwho\s+(made|powers|built|runs)\s+you\b|"
        r"\bare\s+you\s+(a\s+)?(gpt|chatgpt|openai|claude|anthropic|gemini|ollama|qwen|llama|mistral|deepseek|mini\b)|"
        r"\bwhat(?:'s|\s+is)\s+(your|the)\s+(model|provider|llm|ai|name)\b|"
        r"\btell\s+me\s+(your|the)\s+(model|provider|name|identity)\b|"
        r"\b(identify|tell\s+me)\s+(the|your)?\s*(model|provider|ai|llm)\b|"
        r"\bwhat\s+ai\b[^.?!\n]{0,40}\bare\s+you\b"
        r")"
    )
    ...
    def _is_identity_question(cls, text: str) -> bool:
        if not text:
            return False
        t = text.strip()
        if len(t) > 400:  # only treat short direct questions
            return False
        return bool(cls._IDENTITY_QUESTION_RE.search(t))
